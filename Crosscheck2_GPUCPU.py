...
使用GluonCV官方配置，官方数据库，GPU+CPU，训练SSD
GPU需要有12GB显存
这可能是另外一个傻主意，因为在试验中，即便CPU分担了较轻的任务，速度还是远慢于GPU。
这导致系统利用效率的下降。
"""
#%% 
...
net = SSD(network = name, base_size = base_size, features = features, 
          num_filters = filters, sizes = sizes, ratios = ratios, steps = steps,
              pretrained=pretrained, classes=classes,ctx=[mx.cpu()] + ctx)
net.initialize(verbose = False,ctx = [mx.cpu()] + ctx)
#net.load_parameters(Models_tmp_Dir + 'ssd_512_resnet50_v1_voc-9c8b225a.params')
net.hybridize(active= True,static_alloc=False,static_shape = False)    

#%% 生成DataLoader
x = mx.nd.zeros(shape=(1, 3, base_size, base_size),ctx=mx.cpu())
...
trainer = mx.gluon.Trainer(all_params,'Adam')
...
for epoch in range(epoch_num): 
    ...
    for ib, batch in enumerate(train_loader):
        ...
        data = split_and_load_WithCPU(batch[0], GPU_list=ctx, batch_axis=0,CPU_percentage= CPU_percentage)
        cls_targets = split_and_load_WithCPU(batch[1], GPU_list=ctx, batch_axis=0,CPU_percentage=CPU_percentage)
        box_targets = split_and_load_WithCPU(batch[2], GPU_list=ctx, batch_axis=0,CPU_percentage=CPU_percentage)
